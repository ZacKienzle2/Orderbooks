...
def render(log: EventLog, *, px_bins: int = 80, time_bins: int = 120,
    ...
    h, _, _ = np.histogram2d(
        fills["px"].to_numpy(), fills["seq"].to_numpy(),
        bins=[px_edges, seq_edges], weights=fills["qty"].to_numpy(),
    )

    fig, ax = plt.subplots(figsize=(10, 5))
    im = ax.imshow(
        h, aspect="auto", origin="lower", interpolation="nearest",
        extent=(seq_edges[0], seq_edges[-1], px_edges[0], px_edges[-1]), cmap="viridis",
    )
