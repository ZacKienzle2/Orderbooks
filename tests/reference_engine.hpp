...
struct reference_engine {
    ...
    [[nodiscard]] qty_t aggregate(side s, tick_t px) const noexcept {
        if (s == side::bid) {
            ...
            qty_t sum = 0;
            for (auto const& r : it->second) sum += r.remaining;
            return sum;
        }
    ...
    void on_submit(submit_msg const& m) noexcept {
        if (m.s == side::bid) handle_<side::bid>(m);
        else                  handle_<side::ask>(m);
        publish_top_();
    }
    ...
    void on_cancel(cancel_msg const& m) noexcept {
        ...
        rec* r = it->second;
        idx.erase(it);
        if (r->s == side::bid) erase_from_<side::bid>(r);
        else                   erase_from_<side::ask>(r);
        top_dirty = true;
        publish_top_();
    }
    ...
    void on_modify(modify_msg const& m) noexcept {
        ...
        if (m.new_px == r->px) {
            if (m.new_qty == r->remaining) return;
...
private:
    template <side Side>
    void handle_(submit_msg const& m) noexcept {
        if (m.t == tif::fok && !can_fully_fill_<Side>(m.px, m.qty)) return;
        qty_t remaining = m.qty;
        match_<Side>(m, remaining);
        if (remaining == 0) return;
        if (m.t == tif::ioc || m.t == tif::fok) return;
    ...
    [[nodiscard]] bool can_fully_fill_(tick_t aggressor_px, qty_t want) const noexcept {
        ...
        if constexpr (Side == side::bid) {
            for (auto const& [px, fifo] : asks) {
                if (px > aggressor_px) break;
                for (auto const& r : fifo) total += r.remaining;
                if (total >= want) return true;
            }
        } else {
            for (auto const& [px, fifo] : bids) {
                if (px < aggressor_px) break;
                for (auto const& r : fifo) total += r.remaining;
                if (total >= want) return true;
            }
    ...
    template <side Side>
    void match_(submit_msg const& m, qty_t& remaining) noexcept {
        auto crosses = [&](tick_t opp_px) {
            if constexpr (Side == side::bid) return m.px >= opp_px;
            else                             return m.px <= opp_px;
        };
        while (remaining > 0) {
            auto best_opt = (Side == side::bid) ? best_ask() : best_bid();
            if (!best_opt.has_value() || !crosses(*best_opt)) return;
            const auto best_px = *best_opt;
            auto&      fifo    = (Side == side::bid)
                                  ? asks.find(best_px)->second
                                  : bids.find(best_px)->second;
            while (remaining > 0 && !fifo.empty()) {
                ...
                if (m.account_id != 0 && maker.account_id == m.account_id) {
                    if (dispatch_self_cross_<Side>(m, maker, fifo, best_px, remaining)) {
                        if (fifo.empty()) drop_empty_level_<Side>(best_px);
                        return;
                    }
                    continue;
                }
    ...
    template <side Side>
    bool dispatch_self_cross_(submit_msg const& m,
                              rec&              maker,
                              std::list<rec>&   fifo,
                              tick_t            best_px,
    ...
    void drop_empty_level_(tick_t px) noexcept {
        if constexpr (Side == side::bid) asks.erase(px);
        else                             bids.erase(px);
    }
    ...
    void erase_from_(rec* r) noexcept {
        if constexpr (Side == side::bid) {
            ...
            for (auto fit = it->second.begin(); fit != it->second.end(); ++fit) {
                if (&*fit == r) { it->second.erase(fit); break; }
            }
    ...
    void publish_top_() noexcept {
        ...
        if (cfg.top_throttle && have_top &&
            bid_px == last_bid_px && ask_px == last_ask_px &&
