...
void replay(fast_t& fast, ref_t& ref, gen_state& g, std::uint8_t n_accounts) {
    ...
    for (std::size_t i = 0; i < cmd_qty; ++i) {
        ...
        if (g.live.empty()) {
            ...
            if (roll < 6)      k = op_kind::submit;
            else if (roll < 8) k = op_kind::cancel;
            else               k = op_kind::modify;
        }
        switch (k) {
            case op_kind::submit: {
...
void compare_fills(std::vector<lob::fill_msg> const& a, std::vector<lob::fill_msg> const& b) {
...
void compare_trades(std::vector<lob::trade_msg> const& a, std::vector<lob::trade_msg> const& b) {
    REQUIRE(a.size() == b.size());
    for (std::size_t i = 0; i < a.size(); ++i) {
        REQUIRE(a[i].px  == b[i].px);
        REQUIRE(a[i].qty == b[i].qty);
...
void compare_self_trades(std::vector<lob::self_trade_msg> const& a,
                         std::vector<lob::self_trade_msg> const& b) {
    REQUIRE(a.size() == b.size());
    for (std::size_t i = 0; i < a.size(); ++i) {
...
void compare_tops(std::vector<lob::top_msg> const& a, std::vector<lob::top_msg> const& b) {
    REQUIRE(a.size() == b.size());
    for (std::size_t i = 0; i < a.size(); ++i) {
...
void compare_book_state(fast_t const& fast, ref_t const& ref) {
...
TEST_CASE("engine matches reference under self_cross cancel_newest", "[engine][differential][self-cross]") {
...
TEST_CASE("engine matches reference under self_cross decrement_trade", "[engine][differential][self-cross]") {
    auto seed = GENERATE(0xC0FFEEULL, 0xBADC0DEULL, 0xDEADBEEFULL);
