...
lob::qty_t resting_total_on_book(eng_t const& eng) noexcept {
    lob::qty_t total = 0;
...
lob::qty_t order_remaining_or_zero(eng_t const& eng, lob::order_id_t id) noexcept {
...
TEST_CASE("engine GTC stream preserves quantity conservation", "[engine][invariant]") {
    ...
    for (auto const& f : pub.fills) sum_filled += f.qty;
    const auto sum_resting = resting_total_on_book(eng);

    INFO("seed=" << seed
         << " submitted=" << sum_submitted
         << " filled=" << sum_filled
...
TEST_CASE("engine maintains aggregate / bitmap consistency across a mixed stream", "[engine][invariant]") {
    ...
    auto check_consistency = [&] {
        for (lob::tick_t p = 0; p < ticks; ++p) {
            for (auto s : {lob::side::bid, lob::side::ask}) {
                const auto agg = (s == lob::side::bid)
                                     ? eng.book_view().bids().aggregate_at(p)
                                     : eng.book_view().asks().aggregate_at(p);
                const auto& lvl = (s == lob::side::bid)
                                      ? eng.book_view().bids().level_at(p)
                                      : eng.book_view().asks().level_at(p);
                lob::qty_t sum = 0;
                for (auto const& o : lvl.fifo) sum += o.remaining;
                REQUIRE(agg == sum);
