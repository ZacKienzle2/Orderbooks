...
Architectural decisions referenced below live in
[`docs/adr/`](docs/adr/README.md). The roadmap is intentionally aspirational.
Items are not commitments. Priorities shift as the project learns from users and
contributors.
...
- Threaded shard runtime (see
  [ADR-0019](docs/adr/0019-threaded-shard-runtime-core-pinned-workers.md))
  running one worker per shard, each pinned to its own core, draining a
  dedicated SPSC ingress ring into its engine; host tuning documented in
  [`docs/dev/threading.md`](docs/dev/threading.md).
- Per-shard egress rings (see
  [ADR-0020](docs/adr/0020-per-shard-egress-rings.md)) so each shard publishes
  events into its own SPSC ring through a `ring_publisher`, removing the
...
- Publisher-concept seam (see
  ...
  the JSON Lines recorder.
- Huge-page-backed slab arena (see
  [ADR-0023](docs/adr/0023-hugepage-backed-arena.md)) preferring 2 MiB pages to
  cut data-TLB pressure during bursts, with a transparent fallback chain and no
  change to the first-touch NUMA policy.
- From-scratch HDR latency histogram (see
  [ADR-0024](docs/adr/0024-hdr-latency-histogram.md)) with O(1) allocation-free
  record and exact p50 / p99 / p99.9 / p99.99 queries, the in-process backbone
...
- Batched shard-worker quiescence counter (see
  [ADR-0032](docs/adr/0032-batch-worker-processed-counter.md)) replacing a
  per-command release fetch_add with one per drained batch, removing about 20
  reference cycles per command from the busiest loop while preserving the drain
  handshake.
- Randomized engine torture test (`tests/test_engine_stress.cpp`) auditing
  structural invariants over a large mixed command stream under every self-cross
...
- Guarded top-of-book recompute (see
  ...
  away from the top no longer recomputes both sides' best, since a price test
  proves it cannot move the top. Differential-proven byte-identical under both
  throttle settings, and an A/B measured about 24 percent fewer cycles and 22
  percent fewer instructions on the cancel-and-submit path.
- In-place relink for resting price-move modify (see
  [ADR-0028](docs/adr/0028-in-place-modify-relink.md)). When a price change does
  not cross, the existing record is relinked between levels instead of cancelled
  and resubmitted, skipping the index erase and insert and the arena free and
  allocate. Differential-proven byte-identical, and an A/B measured about 2.7x
  fewer cycles and 2.2x fewer instructions on the random resting-move path.
- Match-sweep prefetch measured and reverted (see
...
- Post-only and pegged time-in-force variants (preceded by a top-of-book
  listener seam).
...
- Zero-copy FIX 4.4 tag-value parser over `std::span<const std::byte>`: done
  (`include/lob/fix_parser.hpp`, ADR-0018) for the NewOrderSingle /
  OrderCancelRequest / OrderCancelReplaceRequest order-entry subset, emitting
  `lob::command`. Remaining: gateway adapter writing parsed commands into the
  command ring, and a libFuzzer harness for malformed inputs.
...
- AVX-512 fast-path for bitmap scans. Analysed and declined. The hierarchical
  bitmap (ADR-0005) already resolves best-price in O(tier-count) through
