...
- End-to-end load harness (see [ADR-0030](docs/adr/0030-end-to-end-load-harness.md), `apps/loadgen`) driving the assembled multi-shard runtime under synthetic flow, reporting sustained throughput and a closed-loop unloaded end-to-end latency over the full ingress, worker, match, egress, and merge path. No market data required.
- Binary order-entry gateway (see [ADR-0031](docs/adr/0031-binary-order-entry-gateway.md), `apps/gateway`) terminating a fixed-size order protocol over TCP, decoding each record into an engine command and acknowledging it, busy-polled with a built-in correctness self-test. Makes the engine a connectable endpoint.
- Batched shard-worker quiescence counter (see [ADR-0032](docs/adr/0032-batch-worker-processed-counter.md)) replacing a per-command release fetch_add with one per drained batch, removing about 20 reference cycles per command from the busiest loop while preserving the drain handshake.
- Randomized engine torture test (`tests/test_engine_stress.cpp`) auditing structural invariants over a large mixed command stream under every self-cross policy and throttle setting. The audit checks that the bitmap's populated set equals the non-empty levels, each level's aggregate equals the sum of its FIFO, no resting order has zero remaining or a stray price, and best-of-book is the extreme populated level. Runs under the sanitizer presets; a heavier offline run cleared 48 million ops per configuration.
- Guarded top-of-book recompute (see [ADR-0029](docs/adr/0029-guard-top-recompute.md)). A submit, cancel, or modify away from the top no longer recomputes both sides' best, since a price test proves it cannot move the top. Differential-proven byte-identical under both throttle settings, and an A/B measured about 24 percent fewer cycles and 22 percent fewer instructions on the cancel-and-submit path.
- In-place relink for resting price-move modify (see [ADR-0028](docs/adr/0028-in-place-modify-relink.md)). When a price change does not cross, the existing record is relinked between levels instead of cancelled and resubmitted, skipping the index erase and insert and the arena free and allocate. Differential-proven byte-identical, and an A/B measured about 2.7x fewer cycles and 2.2x fewer instructions on the random resting-move path.
