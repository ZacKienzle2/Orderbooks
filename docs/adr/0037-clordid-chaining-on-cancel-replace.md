...
assigns its next identity in ClOrdID(11). The parser read tag 11 and dropped it,
so after a replace every later request naming the order by its new id silently
no-oped. The engine had no way to express the rename.
...
before dispatching the px and qty branches, so the crossing cancel plus resubmit
also runs under the new identity. The parser requires tag 11 on `G` and carries
it through. Id uniqueness stays the caller's contract, exactly as for submit
ids.
...
- Positive: replace-then-cancel-by-new-id works; the differential stream chains
  a quarter of its modifies to keep it honest.
