...
`NNNN-short-title-kebab-case.md`, where `NNNN` is a 4-digit zero-padded sequence
number assigned at creation time and never reused. Examples:
...
Never edit an Accepted ADR to change its outcome. Write a new ADR that
supersedes it and update the old one's status. This preserves the decision
history.
