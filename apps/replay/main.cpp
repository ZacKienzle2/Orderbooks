...
args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (s == "--seed")       a.seed       = std::stoull(next("--seed"), nullptr, 0);
        else if (s == "--commands")   a.commands   = std::stoul(next("--commands"));
        else if (s == "--output")     a.out_path   = next("--output");
        else if (s == "--accounts")   a.accounts   = static_cast<std::uint32_t>(std::stoul(next("--accounts")));
        else if (s == "--self-cross") a.self_cross = std::stoi(next("--self-cross"));
        else if (s == "--help")       usage(0);
