...
class json_recorder {
  public:
    ...
    void publish(const fill_msg& m) noexcept {
        out_ << R"({"kind":"fill","seq":)" << m.seq << R"(,"maker":)" << m.maker << R"(,"taker":)"
             << m.taker << R"(,"px":)" << m.px << R"(,"qty":)" << m.qty << "}\n";
    }

    void publish(const top_msg& m) noexcept {
        out_ << R"({"kind":"top","seq":)" << m.seq << R"(,"bid_px":)" << m.bid_px << R"(,"ask_px":)"
             << m.ask_px << R"(,"bid_qty":)" << m.bid_qty << R"(,"ask_qty":)" << m.ask_qty << "}\n";
    }

    void publish(const trade_msg& m) noexcept {
        out_ << R"({"kind":"trade","seq":)" << m.seq << R"(,"px":)" << m.px << R"(,"qty":)" << m.qty
             << "}\n";
    }

    void publish(const self_trade_msg& m) noexcept {
        out_ << R"({"kind":"self_trade","seq":)" << m.seq << R"(,"aggressor":)" << m.aggressor
             << R"(,"resting":)" << m.resting << R"(,"account":)" << m.account << R"(,"px":)"
             << m.px << R"(,"qty":)" << m.qty << "}\n";
    }
