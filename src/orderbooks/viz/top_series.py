...
def render(log: EventLog, output: str | Path | None = None) -> Figure:
    ...
    ax_px.step(
        bid["seq"], bid["bid_px"], where="post", color="#2E7D32", label="best bid"
    )
    ax_px.step(
        ask["seq"], ask["ask_px"], where="post", color="#C62828", label="best ask"
    )
    ...
    ax_spread.fill_between(
        tops["seq"], 0, spread, step="post", alpha=0.2, color="#37474F"
    )
