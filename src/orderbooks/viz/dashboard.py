...
Run: `uv run streamlit run src/orderbooks/viz/dashboard.py -- --log artifacts/sim.jsonl`
"""
...
from orderbooks.viz import bitmap_occupancy, depth, event_log, flow_heatmap, top_series
...
def main() -> None:
    ...
    log_path = st.sidebar.text_input(
        "Event log path", str(args.log) if args.log else ""
    )
    if not log_path:
